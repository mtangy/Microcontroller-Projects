...
export   LCD_2_WriteData
export  _LCD_2_WriteData

export   LCD_2_Control
export  _LCD_2_Control

export  LCD_2_PrString
export _LCD_2_PrString

export  LCD_2_PrCString
export _LCD_2_PrCString

export  LCD_2_Position
export _LCD_2_Position

export  LCD_2_PrHexByte
export _LCD_2_PrHexByte

export  LCD_2_PrHexInt
export _LCD_2_PrHexInt

export  LCD_2_Delay50uTimes
export _LCD_2_Delay50uTimes

export  LCD_2_Delay50u
export _LCD_2_Delay50u
...
export  LCD_2_InitBG
export _LCD_2_InitBG

export  LCD_2_InitVBG
export _LCD_2_InitVBG
...
;
; Fastcall16 versions:
export _LCD_2_DrawVBG
export _LCD_2_DrawBG

IF SYSTEM_SMALL_MEMORY_MODEL
; Legacy Fastcall versions:
export  LCD_2_DrawVBG
export  LCD_2_DrawBG
...
;
export   LCD_2_Write_Data
export  _LCD_2_Write_Data
...
LCD_2_Port:           equ    PRT0DR
LCD_2_PortMode0:      equ    PRT0DM0
LCD_2_PortMode1:      equ    PRT0DM1

LCD_2_E:              equ    10h
LCD_2_RW:             equ    40h
LCD_2_RS:             equ    20h

LCD_2_DATA_MASK:      equ    0Fh
LCD_2_READY_BIT:      equ    08h

LCD_2_DATA_READ:      equ    ( LCD_2_E | LCD_2_RW | LCD_2_RS )
LCD_2_CNTL_READ:      equ    ( LCD_2_E | LCD_2_RW )
LCD_2_PORT_WRITE:     equ    7Fh
LCD_2_PORT_MASK:      equ    7Fh

LCD_2_DISP_INC:       equ    03h
LCD_2_DISP_OFF:       equ    08h
LCD_2_DISP_ON:        equ    0Ch
LCD_2_4BIT_2LINE:     equ    2Ch
...
LCD_2_BG_CHAR_WIDTH:  equ    16     ; 16 characters in width
LCD_2_BG_SEG_WIDTH:   equ    80     ; 16 * 5 = 80
LCD_2_BG_COL_START:   equ     0     ; Always start in the left most column

                                  ; Offsets for 2x16, 2x20, 4x20
                                  ; Change these values for a custom LCD

LCD_2_ROW1_OFFSET:    equ    80h    ; Address/command offset for row 1
LCD_2_ROW2_OFFSET:    equ    C0h    ; Address/command offset for row 2
LCD_2_ROW3_OFFSET:    equ    94h    ; Address/command offset for row 1
LCD_2_ROW4_OFFSET:    equ    D4h    ; Address/command offset for row 2

LCD_2_BG_ROW1_OFFSET: equ    80h    ; Address/command offset for row 1
LCD_2_BG_ROW2_OFFSET: equ    C0h    ; Address/command offset for row 2

LCD_2_CG_RAM_OFFSET:  equ    40h    ; Offset to character RAM

AREA UserModules (ROM, REL)

.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_2_PrCString
;
;  DESCRIPTION:
...
.LCD_PrCString_WR:
    call  LCD_2_WriteData              ; Write data to LCD
...
_LCD_2_PrHexByte:
    ...
    index LCD_2_HEX_STR                  ; Get Hex value
    call  LCD_2_WriteData              ; Write data to screen
    pop   A                            ; Restore value
...
_LCD_2_PrHexInt:
    RAM_PROLOGUE RAM_USE_CLASS_1
    swap  A,X
    call  LCD_2_PrHexByte              ; Print MSB
    mov   A,X                          ; Move LSB into position
    call  LCD_2_PrHexByte              ; Print LSB
    RAM_EPILOGUE RAM_USE_CLASS_1
    ret
.ENDSECTION
...
_LCD_2_PrString:
 ...
 .Loop_PrString:
    ...
    ;LCD_2_writeData is known not to modify X so no need to preserve
    call  LCD_2_WriteData              ; Write data to screen
...
;
 LCD_2_WriteData:
_LCD_2_WriteData:
 LCD_2_Write_Data:   ; Do not use
_LCD_2_Write_Data:   ; Do not use
    RAM_PROLOGUE RAM_USE_CLASS_1
    call  LCD_2_Check_Ready            ; Make sure controller is ready
                                       ; A is preserved in LCD_2_Check_Ready
    ...
    call  LCD_2_WDATA_Nibble           ; Write Upper nibble
    pop   A                            ; Retrieve copy of character
    and   A,0Fh                        ; Mask off high nibble
    nop
    nop
    nop
    call  LCD_2_WDATA_Nibble   ; Write Lower nibble
    RAM_EPILOGUE RAM_USE_CLASS_1
    ret
.ENDSECTION
...
_LCD_2_Control:
    ...
    call  LCD_2_WCNTL_Nibble           ; Write high nibble
    pop   A                            ; Restore copy of byte
    and   A,0Fh                        ; Mask off high nibble
    nop
    nop
    nop
    call  LCD_2_WCNTL_Nibble             ; Write Lower nibble
    RAM_EPILOGUE RAM_USE_CLASS_1
    ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_2_WCNTL_Nibble
;
;  DESCRIPTION:
...
;
 LCD_2_WCNTL_Nibble:
    ...
    and   [NO_SHADOW],~LCD_2_PORT_MASK
    mov   A,[NO_SHADOW]
    mov   reg[LCD_2_Port],A                      ; Reset control lines

    pop   A
    and   A,LCD_2_DATA_MASK                      ; Make sure no bogus data in MSN
    or    A,LCD_2_E                              ; Bring "E" Enable line high
    or    A,[NO_SHADOW]                          ; OR in bit 7 just
    mov   reg[LCD_2_Port], A                     ; Write data
    mov   [NO_SHADOW],A                          ; Keep shadow register in sync
    nop
    and   A,(~LCD_2_PORT_MASK|LCD_2_DATA_MASK)   ; Disable E signal and leave data on bus.
    mov   [NO_SHADOW],A                          ; Keep shadow register in sync
    mov   reg[LCD_2_Port],A
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_2_WDATA_Nibble
;
;  DESCRIPTION:
...
LCD_2_WDATA_Nibble:
    ...
    RAM_SETPAGE_CUR >NO_SHADOW          ; Set CUR_PP to LCD variable address
    and   [NO_SHADOW],~LCD_2_PORT_MASK
    or    [NO_SHADOW],LCD_2_RS                        ; Raise RS to signify a Data Write
    mov   A,[NO_SHADOW]
    mov   reg[LCD_2_Port],A

    pop   A
    and   A,LCD_2_DATA_MASK                           ; Make sure no bogus data in A[7:4]
    or    A,(LCD_2_E | LCD_2_RS)                      ; Bring "E" Enable line high
    or    A,[NO_SHADOW]                               ; Keep shadow in sync
    mov   reg[LCD_2_Port], A                            ; Write data
    mov   [NO_SHADOW],A                               ; Keep shadow in sync
    NOP
    and   A,(~LCD_2_PORT_MASK|LCD_2_DATA_MASK|LCD_2_RS)  ; Disable E signal and leave Data on bus
    mov   [NO_SHADOW],A                               ; keep shadow in sync
    mov   reg[LCD_2_Port],A
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_2_Check_Ready
;
;  DESCRIPTION:
...
LCD_2_Check_Ready:
    ...
    RAM_SETPAGE_CUR >NO_SHADOW                        ; Set CUR_PP to LCD variable address
    and   [NO_SHADOW],~LCD_2_PORT_MASK                ; Mask of all LCD bits
    mov   A,[NO_SHADOW]
    mov   reg[LCD_2_Port],A                           ; Zero LCD port bits

    and   [NO_SHADOW],~LCD_2_DATA_MASK                ; Clear out LCD mode bits.
    mov   A,[NO_SHADOW]
    M8C_SetBank1                                      ; Change port mode to read status
    mov   reg[LCD_2_PortMode0],A                      ; Setup LCD Port for reading
    M8C_SetBank0

    or    [NO_SHADOW],LCD_2_RW                        ; Raise RW to signify Read operation
    mov   A,[NO_SHADOW]
    mov   reg[LCD_2_Port],A
    NOP
...
.LCD_RDY_LOOP:
    or    [NO_SHADOW], LCD_2_CNTL_READ                          ; Raise E to start cycle
    mov   A,[NO_SHADOW]
    mov   reg[LCD_2_Port],A

    nop                                               ; Wait 2 nops to make sure data is ready
    nop
    mov   A,reg[LCD_2_Port]
...
ENDIF   
    ...
    push  A
    and   [NO_SHADOW],(~LCD_2_PORT_MASK | LCD_2_RW)             ; Lower E signal
    mov   A,[NO_SHADOW]
    mov   reg[LCD_2_Port],A
    ...
    pop   A
    and   A,LCD_2_READY_BIT                                       ; Check busy
...
.UNLOCK:
    ...
    or    [NO_SHADOW],LCD_2_PORT_WRITE                ; Revert Data bit to Write mode
    mov   A,[NO_SHADOW]
    M8C_SetBank1
    mov   reg[LCD_2_PortMode0],A                      ; Setup LCD Port for writing
    M8C_SetBank0
    pop   A
...
_LCD_2_Init:
    ...
    and   [NO_SHADOW],~LCD_2_PORT_MASK                          ; Mask off LCD bits
    or    [NO_SHADOW],LCD_2_PORT_WRITE                          ; Set LCD port for writing
    and   [NO_SHADOW],~LCD_2_PORT_MASK                          ; Mask off LCD bits
    ...
    mov   reg[LCD_2_PortMode0],A                                ; Setup LCD Port for writing
    mov   A,[NO_SHADOW]
    mov   reg[LCD_2_PortMode1],A
    M8C_SetBank0

    mov   A,250                             ; Delay for 12.5 mSec (250 * 50uSec)
    call  LCD_2_Delay50uTimes
    mov   A,250                             ; Delay for 12.5 mSec (250 * 50uSec)
    call  LCD_2_Delay50uTimes

    mov   A,03h
    call  LCD_2_WCNTL_Nibble
    ...
    mov   A,LCD_2_4BIT_2LINE                  ; Setup for 4 bit interface, 2 line
    call  LCD_2_Control

    mov   A,LCD_2_DISP_OFF
    call  LCD_2_Control

    mov   A,LCD_2_DISP_ON
    call  LCD_2_Control

    mov   A,LCD_2_DISP_INC
    call  LCD_2_Control
...
.LITERAL
LCD_2_ROW_OFFSET::
     DB    LCD_2_ROW1_OFFSET, LCD_2_ROW2_OFFSET, LCD_2_ROW3_OFFSET, LCD_2_ROW4_OFFSET
.ENDLITERAL
...
_LCD_2_Position:
    ...
    push  X                            ; Store COL
    index LCD_2_ROW_OFFSET ; Get ROW memory offset from table
    mov   X,SP                         ; Get Stack pointer
    add   A,[X+(-1)]                   ; Add the COL to the display pointer
    pop   X

    call  LCD_2_Control                ; Write control byte
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_2_Delay50uTimes
;
;  DESCRIPTION:
...
;
 LCD_2_Delay50uTimes:
_LCD_2_Delay50uTimes:
    RAM_PROLOGUE RAM_USE_CLASS_1
    call  LCD_2_Delay50u
    dec   A
    jnz   LCD_2_Delay50uTimes
    RAM_EPILOGUE RAM_USE_CLASS_1
    ret
...
.SECTION
 ...
 _LCD_2_Delay50u:                      ; [11]  Call
    ...
    cmp   A,05h
    jnc   .Delay50u_End
    index LCD_2_Delay50u_Table ; [13] Get delay value
.Delay50u_Loop:                          ;
    dec   A                              ; [4]
    jnz   .Delay50u_Loop                 ; [5]
.Delay50u_End:
...
;  ** function in assembly you should convert to _LCD_2_DrawVBG
...
;    functions.
;
;    If LCD_2_Init is not called before this function, the
...
LCD_2_DrawBG:
    ...
    call  LCD_2_Position               ; Set cursor position
...
.LCD_BG_DOITX:
    call  LCD_2_WriteData              ; Display BG character
...
.VBG_LOOPX:
    push  A
    index LCD_2_ROW_OFFSET  ; Get row offset
    add   A,[X+VBG_COLX]                ; Add column offset to position
    call  LCD_2_Control                ; Position Cursor
...
.VBG_WRITE_CHARX:                        ; Write character to display
    call  LCD_2_WriteData              ; Write value
    pop   A
    dec   A
...
_LCD_2_InitVBG:
    ...
    mov   A,LCD_2_CG_RAM_OFFSET              ; Setup pointer
    call  LCD_2_Control                ; Position the CG pointer
...
.VBG_Load:
    call  LCD_2_WriteData              ; character data
    ...
    pop  A
    pop  A
    mov  A,LCD_2_DISP_ON                    ; Turn on display, don't really
    call LCD_2_Control                 ; need this.
    RAM_EPILOGUE RAM_USE_CLASS_2
    ret
.ENDSECTION

;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_2_InitBG
;
;  DESCRIPTION:
...
_LCD_2_InitBG:
 ...
 .BG_Loop2:
    ...
    jnz   .BG_OTHER
    index LCD_2_BG_TYPE1
    jmp   .BG_Load
 .BG_OTHER:
    index LCD_2_BG_TYPE2
 .BG_Load:
    call  LCD_2_WriteData
